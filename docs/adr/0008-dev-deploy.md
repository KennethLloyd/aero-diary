...
- **Dev**: Mac-first. Clone from GitHub origin, `pnpm dev` against a local SQLite file, and run ChatMock locally for polish. `.env.local` carries the Drive refresh token + session secret + `LLM_BASE_URL=http://127.0.0.1:8000`.
