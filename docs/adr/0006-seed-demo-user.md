...
The configured demo lets visitors explore Aero Diary without exposing private account data. The demo must look alive: calendar with mood dots, insights with distributions, timeline with variety. A demo account must not be a special database user type because it exercises the same auth and data paths as any other account.
...
- `pnpm db:seed` is the canonical first-time and repeat setup command. It reads server-only `DEMO_EMAIL` and `DEMO_PASSWORD`, provisions or updates that ordinary user, and creates:
  ...
  - **90 days of fictional entries** ending "today", covering all 5 moods (weighted like a real life: mostly Good/Rad, some Meh, rare Bad/Awful), ~8 demo-owned activities, a handful of stock photos.
  - Fictional-but-plausible content (no real people, places, or events from a private account).
