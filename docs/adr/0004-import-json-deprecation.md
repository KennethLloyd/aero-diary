...
- **Activities**: the import script owns the 25 legacy strings and resolves them into `Activity` rows belonging to the imported private account, with the curated emoji map, case-insensitively and idempotently. The Activities screen owns CRUD and archival after rows exist; it does not seed the legacy vocabulary.
...
- Activity vocabularies are user-scoped; the import must never reuse or mutate the configured demo account's activity rows.
