...
An import file is an input to Aero Diary. The import workflow must accept any structurally valid template without requiring knowledge of its origin, private content, filenames, or fixed historical count.
...
- Import input data remains outside the repository and is never needed by agents or development fixtures.
- The importer is reusable for any structurally valid input and remains safe as the journal grows.
- The application schema is the destination for successfully imported rows.
