...
- **Activities**: the import script owns the 25 legacy strings and resolves them into `Activity` rows belonging to the imported real user, with the curated emoji map, case-insensitively and idempotently. The Activities screen owns CRUD and archival after rows exist; it does not seed the legacy vocabulary.
...
- Activity vocabularies are user-scoped; the import must never reuse or mutate the demo user's activity rows.
