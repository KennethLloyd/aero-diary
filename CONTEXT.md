...
A self-hosted mood journal ("your memories, vividly preserved") in the Frutiger Aero aesthetic — a Daylio alternative for one private user plus a configured demo account. Next.js 16 + SQLite, deployed on Oracle Cloud (prod) and developed on macOS.
...
- **Activity** — a user-owned emoji + name tag (e.g. `💻 Work`, `🌲 Trail`), CRUD-manageable in the Activities screen, attachable to that user's entries. The private and configured demo accounts have independent activity vocabularies. Legacy Daylio activities and their curated emoji map are owned by the import workflow; the Activities screen does not seed them.
