...
`.env.example` is the configuration template and the authoritative list of supported variables. Copy it to `.env.local` for local development or to the environment used to run the application. All variables are read server-side; secrets never use `NEXT_PUBLIC_*` variables.

| Variable | Required | Purpose |
| --- | --- | --- |
| `DATABASE_URL` | Yes | SQLite URL. Use `file:./data/aero-diary.db` for local development. |
...
Use `pnpm db:migrate` for local schema development and `pnpm db:migrate:deploy` for an already-reviewed migration.
...
Provide the required environment variables, protect access to the application, use persistent SQLite storage, and back up that storage securely.
...
- Use `package.json`, `.env.example`, and `.github/workflows/` as the sources of truth for scripts, configuration, and CI.
